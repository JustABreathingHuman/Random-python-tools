...
def traingeneration(network, links, random, population, samplesize, log):
    linkscache = []
    linkscache.append(links) #caches base links
    networkscorecache = []
    testscache = []
    score = 0
    for i in range(samplesize):
        testscache.append(generatedata())
        networkout = networksolvebinary(network, links, testscache[i])
        decoded = decode(testscache[i])
        correct = solvetobinary(decoded)
        score = score + compare(correct, networkout)
    networkscorecache.append(score)
    for i in range(population):
        links = linkscache[0] #resets to base links
        links = train(network, links, random)
        score = 0
        for n in range(samplesize):
            networkout = networksolvebinary(network, links, testscache[n])
            decoded = decode(testscache[n])
            correct = solvetobinary(decoded)
            score = score + compare(correct, networkout)
        linkscache.append(links)
        networkscorecache.append(score)
    biggest = 0
    chosenindex = 0
    for i in range(len(linkscache)):
        if networkscorecache[i] > biggest:
            chosenindex = i
    output = []
    output.append(linkscache[chosenindex])
    if log == 1:
        output.append(networkscorecache[chosenindex] / samplesize)
    return(output)
...
def trainnetwork(network, links):
    ...
    for i in range(int(generations)):
        ...
        links = output[0]
        benchmark = networksolvebinary(network, links, test)
        benchmark = decode(benchmark)
        print('gen ',i + 1,'. average score: ',output[1],'. currently, 1 + 1 is: ',benchmark)
    run(network,links)
